...
const mapTwoToThe = (input) => {
  let result = input.map(value => Math.pow(2, value));
  return result;
};
...
const charCode = (input) => {
  let result = input.map(character => character.charCodeAt());
  return result;
};
...
const extractAbilities = (abilities) => {
  let result = abilities.map(obj => obj.ability.name);
  //let result = abilities.map(Object.values(ability)[2].name);
  return result;
};
...
const extractStats = (stats) => {
  let results = stats.map(obj => {
    let reformattedObj = {};
    reformattedObj['name'] = obj.stat.name;
    reformattedObj['total'] = obj.effort + obj.baseStat;
